...
import { partyPacks } from "@/lib/party-cards";
import { getRandomItem, cn } from "@/lib/utils";
import type { GameCard } from "@/types";

export function DrinkingGame() {
  const { session, players, addToHistory, addScore, nextTurn, setPhase } = useGameStore();
  ...
  const pack = partyPacks.find((p) => p.id === "drinking-games");
  const allCards = pack?.cards || [];

  const drawCard = useCallback(() => {
    const available = allCards.filter((c) => !playedIds.includes(c.id));
    const card = available.length > 0 ? getRandomItem(available) : getRandomItem(allCards);
    setActiveCard(card);
  ...
  const needsVoting = activeCard && activeCard.tags.includes("party") &&
    (activeCard.text.toLowerCase().includes("who") || activeCard.text.toLowerCase().includes("everyone") || activeCard.text.toLowerCase().includes("rank"));
    ...
    <div className="space-y-4 sm:space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-[10px] sm:text-xs text-white/40">
            {currentPlayer?.name}&apos;s turn • {session?.level?.toUpperCase()}
          </p>
        </div>
          ...
          >
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs font-bold tracking-wider text-amber-400">
                {activeCard.type.toUpperCase().replace("-", " ")}
              </span>
              <span className="ml-auto text-xs text-neon-pink font-bold">
            ...
            {activeCard.choices && activeCard.choices.length > 0 && (
              <div className="space-y-2">
                {activeCard.choices.map((choice, i) => (
                  <div key={i} className="bg-blood-800/50 rounded-xl p-3 text-center">
                    <p className="text-sm font-bold text-white">{choice}</p>
                  </div>
                ))}
              </div>
            )}
            ...
            <div className="flex gap-3">
              {needsVoting ? (
                <Button onClick={handleVoting} variant="primary" className="flex-1 min-h-[48px]">
                  🗳️ Start Voting
                </Button>
              ) : (
                <Button onClick={handleAccept} variant="primary" className="flex-1 min-h-[48px]">
                  ✅ Done / Drink!
                </Button>
              )}
              <Button onClick={handleNext} variant="ghost" className="min-h-[48px]">
                ⏭️ Skip
              </Button>
            </div>
          </motion.div>
            ...
            <div className="grid grid-cols-2 gap-2">
              {players.map((player) => (
                <motion.button
                  ...
                  className={cn(
                    "p-3 rounded-xl border text-center transition-all min-h-[60px]",
                    votes[player.id]
                ...
                >
                  ...
                  {votes[player.id] && (
                    <span className="text-[10px] text-neon-pink">{votes[player.id]} votes</span>
                  )}
                </motion.button>
              ))}
            </div>
            <Button onClick={finishVoting} variant="danger" className="w-full min-h-[48px]">
              🔒 Lock In Votes
            </Button>
          </motion.div>
        )}
      ...
      <AnimatePresence>
        {showResult && !voting && (
          ...
          >
            ...
            <Button onClick={handleNext} variant="primary" className="min-h-[48px]">
              Next Player →
            </Button>
          </motion.div>
        )}
      ...
      {!activeCard && !showResult && !voting && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center">
          <Button onClick={drawCard} variant="danger" size="lg" className="min-h-[52px]">
            🍺 Draw a Card
          </Button>
        </motion.div>
      )}
      ...
      <div className="pt-4 border-t border-white/10">
        <Button variant="ghost" size="sm" onClick={() => setPhase("aftercare")} className="w-full min-h-[44px]">
          🛑 End Session
        </Button>
      </div>
    </div>
