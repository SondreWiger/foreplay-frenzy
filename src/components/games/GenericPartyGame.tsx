...
export function GenericPartyGame({ gameType }: GenericPartyGameProps) {
  ...
  const packMap: Record<GameType, string> = {
    "most-likely": "whos-most-likely",
    "would-you-rather": "would-you-rather-spicy",
  ...
  const pack = partyPacks.find((p) => p.id === packMap[gameType]);
            ...
            <div className="flex gap-3">
              {gameType === "charades" && !timerActive ? (
                <Button onClick={handleAccept} variant="primary" className="flex-1 min-h-[48px]">
                  ⏱️ Start Timer ({activeCard.duration}s)
                </Button>
              ) : gameType === "charades" && timerActive ? (
                <Button onClick={handleCharadesDone} variant="danger" className="flex-1 min-h-[48px]">
                  ✅ Guessed It!
                </Button>
              ) : gameType === "two-truths" || gameType === "story" ? (
                <Button onClick={handleAccept} variant="primary" className="flex-1 min-h-[48px]">
                  ✅ Done
                </Button>
              ) : (
